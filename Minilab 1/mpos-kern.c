...
//
//         /-+----------------+------------+------------+------------+---/
//           |   Application  | Miniproc 1 | Mi		proc_array[i].waitq = 0;niproc 2 | Miniproc 3 |
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		//added for Excercise 3 - begin
		if (current->waitq > 0) {
			proc_array[current->waitq].p_state = P_RUNNABLE;
			proc_array[current->waitq].p_registers.reg_eax = current->p_exit_status;
		}
		current->p_state = P_EMPTY;
		//added for Excercise 3 - end
		schedule();
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			//added for Excercise 3 - begin
			proc_array[p].waitq = current->p_pid;
...
do_fork(process_t *parent)
{
	...
	//added for Excercise 2 - begin
	int counter = 0;
	pid_t pid = current->p_pid;
	uint32_t cur_stack_size;

	while (counter <= NPROCS) {
		pid = (pid + 1) % NPROCS;
		if (proc_array[pid].p_state == P_EMPTY && pid)
			break;
		counter++;
	}
	...
	memcpy(newproc, parent, sizeof(process_t)); //instead of =, extra sure of pointers

	//Initialize fields
	newproc->p_pid = pid; //making sure that this is correct
	newproc->p_state = P_RUNNABLE; //ready to run
	//newproc->p_exit_status is undefined since has not yet exited

	copy_stack(newproc, parent);

	newproc->p_registers.reg_eax = 0;
	cur_stack_size = (PROC1_STACK_ADDR + parent->p_pid*PROC_STACK_SIZE) - parent->p_registers.reg_esp;
	newproc->p_registers.reg_esp = (PROC1_STACK_ADDR + newproc->p_pid*PROC_STACK_SIZE) - cur_stack_size;

	return pid;
	//added for Excercise 2 - end
}
...
copy_stack(process_t *dest, process_t *src)
{
	...
	//added for Excercise 2 - begin
	//from MINIPROCOS MEMORY MAP line24
	src_stack_top = PROC1_STACK_ADDR + src->p_pid*PROC_STACK_SIZE;
	dest_stack_top = PROC1_STACK_ADDR + dest->p_pid*PROC_STACK_SIZE;

	cur_stack_size = src_stack_top - src_stack_bottom;
	dest_stack_bottom = dest_stack_top - cur_stack_size;

	memcpy((char *)dest_stack_bottom, (char *)src_stack_bottom, cur_stack_size);
	//added for Excercise 2 - end
}
